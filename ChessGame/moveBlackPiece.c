...
void moveHighestFitness() {
	struct chessPiece* cp = bestPieceToMove();
	struct chessPiece* cpW;

	// checks if black king is threated
		// if it is, move that piece to another valid spot not threatened
			// if no spot exists, end the game
		// otherwise move another piece
	if (is_BlackKingThreatened()) {
		struct chessPiece* cp = &chessPiecesBlack[14];
		// check if there is a place to move the king that has no threat
		if (!movePieceNoThreat(cp)) { 
			cp->is_live = 0; // if not safe places exist, set black king as defeated, ending the game
		}
	}
	else {
		...
		// othewise, move the best piece somewhere, depending on the pieces it can take and protect
			...
			if (cp->protects == 0 && cp->enemiesCanDefeat == 0) {
				if (!moveBlackIfValid(cp)) {
					randomBlackMove();
				}
			}
			else {
